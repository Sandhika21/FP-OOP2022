...
public class Board extends JPanel implements ActionListener, Storage {
    ...
    private void gameOver(Graphics g) {
        ...
        g.drawString(msg+msg1, ((B_WIDTH - metr.stringWidth(msg)) / 2) + 1, B_HEIGHT / 2);
        
        msg = command.get(2);
        msg = C.GetK(msg); 
        msg1 = Integer.toString(count+1);
        g.drawString(msg+msg1, ((B_WIDTH - metr.stringWidth(msg)) / 2) + 2, B_HEIGHT / 2);
        
        msg = command.get(3);
        msg = C.GetK(msg); 
        msg1 = Integer.toString(score);
        g.drawString(msg+msg1, ((B_WIDTH - metr.stringWidth(msg)) / 2) + 3, B_HEIGHT / 2);
    }
    ...
    private void move() {
        ...
        for (int z = locate(); z > 0; z--) {            
            LA.Set(x, z, LA.Set(x, z-1));
            LA.Set(y, z, LA.Set(y, z-1));
        }

        if (isLeftDirection()) {
            IG.Set(x, 0, 0-DOT_SIZE);
        }
